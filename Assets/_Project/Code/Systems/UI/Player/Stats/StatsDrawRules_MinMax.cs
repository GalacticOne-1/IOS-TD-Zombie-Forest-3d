...
{
    [CreateAssetMenu(fileName = "StatsDrawRules_MinMax", menuName = "Game Configs/UI/Stats Draw Rules_MinMax")]
    public class StatsDrawRules_MinMax : StatsDrawRules
    {
        public override void Apply(StatSlotParam param)
        {
            if (param.valueText)
            {
                if (!param.valueText.gameObject.activeInHierarchy || !param.textAnimation)
                    param.valueText.text = $"{param.currValue:0}/{param.maxValue:0}";
